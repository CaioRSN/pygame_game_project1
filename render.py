...
def desenhar_menu(tela, indice_selecionado, mostrar_controles):
    ...
    tela.blit(recursos.sprite_fundo_menu, (0, 0))
    
    tempo = pygame.time.get_ticks()

    largura_original = tela.get_width()
    altura_original = tela.get_height()
    fator_zoom = 1.03 + 0.03 * math.sin(tempo * 0.001) #vai aumentar 3% a imagem
    ...
    fundo_x = (largura_original - nova_largura) // 2 # Centraliza o zoom
    fundo_y = (altura_original - nova_altura) // 2
    fundo_x += int(10 * math.cos(tempo * 0.0008))   # balanço
    
    tela.blit(fundo_com_zoom, (fundo_x, fundo_y))
 

    if mostrar_controles:
        # Caixinha para os textos dos controles
        overlay = pygame.Surface((1000, 470)) #largura e altura da caixinha 
        overlay.set_alpha(210) #transparencia da caixinha
        overlay.fill((0, 0, 0)) #cor/preenchimento
        ...
        for i, linha in enumerate(controles):
            ...
            # Sombra para o texto dos controles
            sombra = recursos.fonte_pixel_titulo.render(linha, True, (20, 20, 20))
            tela.blit(sombra, (tx + 2, ty + 2)) # Desenha a sombra um pouquinho deslocada
        ...
        for i, opcao in enumerate(opcoes):
            tx = largura_original // 2 - 150 # Alinhamento base fixo para não tremer
            ty = 850 + i * 70
            
            # Determina o texto exato (com ou sem setinha)
            tx -= 50

            if i == indice_selecionado:
                # Efeito de deslocamento para a direita ao selecionar
                tx += 30
                
                valor_pulsante = int(197 + 58 * math.sin(tempo * 0.005))  # Efeito Pulsar
                cor_pulsante = (valor_pulsante, int(valor_pulsante * 0.84), 0) # Varia o tom do Dourado
                ...
                texto_final = opcao
                cor_final = cor_normal
                
            # Renderiza o texto definitivo
            ...
            # Desenha primeiro a sombra e depois o texto principal
